...
This is because it will massively increase the sample size (e.g., for individual-level studies).
...
`usgeogr` provides the following choices:[^3]
...
2. `bscp_df` assigns border counties to clusters defined by 50 mile-long strips along state 
borders. Some strips only have a single county or a single state assigned (i.e., the variation will
be ignored in a panel).

3. `cpcp_df` assigns border counties to cross-border clusters using the "couplet" and
"relaxed couplet" algorithms.[^5] The couplet algorithm sequentially assigns counties to clusters 
of size two (i.e., "couples"). The relaxed couplet algorithm allows for more than two counties per
cluster. In both algorithms, counties can only belong to one cluster. 

4. `mxcp_df` assigns border counties to cross-border clusters using the "max-method" and
"relaxed max-method" algorithms.[^6]. The max-method algorithm assigns counties to clusters
based on their centrality and the number of counties they border. In both algorithms, counties 
can only belogn to one cluster. 
...
assigns each ZCTA to its nearest 20 mile long border strip. 
